...
function show_victorina(){
	...
	{count+=1};  
	if (count>0) alert("You have not completed all answers. Be more attentive!")    
	        else answer();
	    }
	} 
	...
	if(nok==5) s="Excellent";
	if(nok<5) s="Good";
	if(nok<3.75) s="Acceptable";
	if (nok<2.5) s="Bad";
	    document.test.s1.
	    value="The amount of correct answers -  "+nok+". Your mark is "+s+". Look at the field with the number of question. If the answer is correct, then there is +, if it is incorrect, then there is -."
